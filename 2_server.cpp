...
int main() {
    ...
    printf("UDP server started on 0.0.0.0:%d\n", SERVER_PORT);
    ...
    while (true) {
        ...
        if (strcmp(buffer, "-echofile") == 0) {
            ...
            sendto(
                sockServer,
                fileInfo,
                static_cast<int>(strlen(fileInfo)) + 1,
                0,
                (SOCKADDR*)&addrClient,
                sizeof(SOCKADDR)
            );
            ...
            bool okSend = gbn::sendBuffer(sockServer, addrClient, localFileData, "SERVER-TX");
            if (!okSend) {
            ...
            bool okRecv = gbn::receiveBuffer(sockServer, addrClient, backSize, echoed, "SERVER-RX");
            if (!okRecv) {
            ...
            if (!gbn::writeFile("server_echo_back.bin", echoed)) {
                printf("failed to save server_echo_back.bin\n");
            }
