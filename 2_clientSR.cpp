...
int main(int argc, char* argv[]) {
    ...
    printf("SR UDP client started. Server = %s:%d\n", serverIp, SERVER_PORT);
    ...
    while (true) {
        ...
        if (strcmp(sendBuf, "-time") == 0 || strcmp(sendBuf, "-quit") == 0) {
            sendto(sockClient, sendBuf, static_cast<int>(strlen(sendBuf)) + 1, 0, (SOCKADDR*)&addrServer, sizeof(addrServer));
        ...
        if (strcmp(sendBuf, "-echofile") == 0) {
            ...
            sendto(sockClient, "READY_RECV", static_cast<int>(strlen("READY_RECV")) + 1, 0, (SOCKADDR*)&addrServer, sizeof(addrServer));

            std::vector<char> recvData;
            bool okRecv = sr::receiveBuffer(sockClient, addrServer, fileSize, recvData, "SR-CLIENT-RX");
            if (!okRecv) {
                printf("receive from server failed\n");
                continue;
            }

            sr::writeFile("client_sr_recv.bin", recvData);

            char readySend[256];
            sprintf_s(readySend, "READY_SEND %zu", recvData.size());
            sendto(sockClient, readySend, static_cast<int>(strlen(readySend)) + 1, 0, (SOCKADDR*)&addrServer, sizeof(addrServer));

            bool okSend = sr::sendBuffer(sockClient, addrServer, recvData, "SR-CLIENT-TX");
            if (!okSend) {
