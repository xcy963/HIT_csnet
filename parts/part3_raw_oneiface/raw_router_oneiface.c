...
#define DEFAULT_MATCH_SRC_IP "192.168.1.1"
#define DEFAULT_MATCH_DST_IP "192.168.1.3"
#define DEFAULT_NEXT_HOP_MAC "02:42:ac:1e:00:31"
...
int main(int argc, char **argv) {
    ...
    if (argc == 5) {
        ...
        match_dst_text = argv[3];
        next_mac_text = argv[4];
    }

    //iface 监听和转发使用的网卡名
    //match_src: 要匹配的源 IP
    uint32_t match_src = parse_ipv4_or_die(match_src_text);
    // match_dst：要匹配的目的 IP
    uint32_t match_dst = parse_ipv4_or_die(match_dst_text);
    
    //解析下一个地址的mac
    uint8_t next_mac[MAC_LEN];
    if (parse_mac(next_mac_text, next_mac) != 0) {
    ...
    */
    ...
    printf("One-interface router on %s. Forwarding %s -> %s to MAC ", iface, match_src_text, match_dst_text);
    print_mac(next_mac);
    printf("\n");
    if (debug_enabled()) {
